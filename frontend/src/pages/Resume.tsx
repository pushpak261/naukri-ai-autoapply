      ...
      <div className="flex items-center gap-2">
        <button
          ...
          className={`px-3 py-1.5 text-sm rounded-lg transition-colors flex items-center gap-1.5 ${
            showVersions ? 'bg-primary/10 text-primary border border-primary/30' : 'border border-border text-secondary hover:bg-surface-hover'
          }`}
        >
        ...
        {!editMode && (
          <div className="flex items-center gap-1.5">
            <input
              ...
              placeholder="Label..."
              className="w-28 px-2 py-1.5 text-xs bg-bg border border-border rounded text-text placeholder-muted outline-none focus:border-primary"
            />
            <button
              onClick={takeSnapshot}
              disabled={!snapshotLabel.trim()}
              className="px-2 py-1.5 text-xs bg-primary/10 text-primary rounded-lg hover:bg-primary/20 disabled:opacity-50"
            >
              Snapshot
            </button>
          </div>
      ...
      {showVersions && versions.length > 0 && (
        <div className="bg-surface rounded-xl border border-border p-4">
          ...
          <div className="space-y-2">
            {versions.map((v) => (
              <div key={v.id} className="flex items-center justify-between p-2 rounded-lg hover:bg-surface-hover/50">
                <div className="flex items-center gap-3">
                  <span className="text-xs font-mono text-primary">{v.label}</span>
                  <span className="text-xs text-muted">{new Date(v.timestamp).toLocaleString()}</span>
                </div>
                <div className="flex gap-1">
                  <button
                    onClick={() => setSelectedVersion(selectedVersion?.id === v.id ? null : v)}
                    className={`px-2 py-1 text-xs rounded ${selectedVersion?.id === v.id ? 'bg-primary/20 text-primary' : 'text-secondary hover:bg-surface-hover'}`}
                  >
          ...
          {diff && diff.length > 0 && (
            <div className="mt-4 border-t border-border pt-3">
              <h4 className="text-xs font-semibold text-text mb-2">Changes ({diff.length})</h4>
              <div className="space-y-2 max-h-64 overflow-y-auto">
                {diff.map((d, i) => (
                  <div key={i} className="p-2 bg-bg rounded-lg">
                    <p className="text-[10px] uppercase text-muted mb-1">{d.field}</p>
                    <div className="grid grid-cols-2 gap-2">
                      <div className="p-1.5 bg-red-500/5 border border-red-500/20 rounded text-xs text-red-300">
                        ...
                        {d.before || <span className="italic text-muted">empty</span>}
                      </div>
                      <div className="p-1.5 bg-green-500/5 border border-green-500/20 rounded text-xs text-green-300">
                        ...
                        {d.after || <span className="italic text-muted">empty</span>}
                      </div>
                    </div>
                  </div>
                ))}
          ...
          {diff && diff.length === 0 && (
            <p className="text-xs text-muted mt-2">No differences between selected versions.</p>
          )}
        </div>
      )}
