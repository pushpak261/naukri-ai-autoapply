    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
        <div className="bg-surface rounded-xl border border-border p-4">
          <h3 className="text-xs font-medium text-secondary flex items-center gap-1.5 mb-2">
          ...
          {session.isLoading ? (
            ...
            <p className="text-sm text-muted">No session</p>
          )}
        </div>
        ...
        <div className="bg-surface rounded-xl border border-border p-4">
          ...
          {metrics.isLoading ? (
            ...
            <div>
              <p className="text-sm text-text">API Calls: {metrics.data.api_calls}</p>
              <p className="text-xs text-muted mt-1">Total runs: {metrics.data.total_runs}</p>
              {metrics.data.duration_seconds > 0 && (
                <p className="text-xs text-muted">Duration: {Math.round(metrics.data.duration_seconds / 60)} min</p>
              )}
            </div>
          ) : (
        ...
        <div className="bg-surface rounded-xl border border-border p-4">
          ...
          <div className="flex flex-wrap gap-1.5">
            <Link to="/agent-control" className="text-xs px-2 py-1 rounded bg-surface-hover hover:bg-surface-hover text-secondary hover:text-text transition-colors">Agent</Link>
            <Link to="/analytics" className="text-xs px-2 py-1 rounded bg-surface-hover hover:bg-surface-hover text-secondary hover:text-text transition-colors">Analytics</Link>
            <Link to="/skills-gap" className="text-xs px-2 py-1 rounded bg-surface-hover hover:bg-surface-hover text-secondary hover:text-text transition-colors">Skills</Link>
            <Link to="/scam-detector" className="text-xs px-2 py-1 rounded bg-surface-hover hover:bg-surface-hover text-secondary hover:text-text transition-colors">Scams</Link>
            <Link to="/cache-explorer" className="text-xs px-2 py-1 rounded bg-surface-hover hover:bg-surface-hover text-secondary hover:text-text transition-colors">Cache</Link>
            <Link to="/log-viewer" className="text-xs px-2 py-1 rounded bg-surface-hover hover:bg-surface-hover text-secondary hover:text-text transition-colors">Logs</Link>
            <Link to="/backups" className="text-xs px-2 py-1 rounded bg-surface-hover hover:bg-surface-hover text-secondary hover:text-text transition-colors">Backups</Link>
          </div>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        <div className="bg-surface rounded-xl border border-border p-5">
          ...
          {runChartData.length > 0 ? (
            ...
            <div className="flex items-center justify-center h-64 text-muted">No run data yet</div>
          )}
        </div>
        ...
        <div className="bg-surface rounded-xl border border-border p-5">
          ...
          {data.stats.total > 0 ? (
            ...
            <div className="flex items-center justify-center h-64 text-muted">No applications yet</div>
          )}
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        <div className="bg-surface rounded-xl border border-border p-5">
          ...
          <div className="space-y-2">
            {data.recent_applications.length > 0 ? (
              ...
              <div className="text-center py-8 text-muted">No recent applications</div>
            )}
          </div>
        </div>
        ...
        <div className="bg-surface rounded-xl border border-border p-5">
          <div className="flex items-center justify-between mb-4">
            ...
            <Link to="/run-logs" className="text-sm text-primary hover:underline">View all</Link>
          </div>
          <div className="space-y-2">
            {data.recent_runs.length > 0 ? (
              data.recent_runs.map((run) => (
                <div key={run.id} className="flex items-center justify-between p-3 rounded-lg bg-bg border border-border">
                  <div className="min-w-0 flex-1">
                    <p className="text-xs text-secondary">{run.started_at.slice(0, 16).replace('T', ' ')}</p>
                    <p className="text-sm text-text truncate mt-0.5">{run.keywords.slice(0, 40)}</p>
                  </div>
                  <div className="flex items-center gap-3 ml-3">
                    <span className="text-xs text-secondary">+{run.applied} / -{run.skipped}</span>
                    <StatusBadge status={run.status} />
                  </div>
                </div>
              ))
            ) : (
              <div className="text-center py-8 text-muted">No runs yet</div>
            )}
          </div>
        </div>
      </div>
